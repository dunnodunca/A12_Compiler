 ...
 * -------------------------------------------------------------
 */
kao_void bErrorPrint(kao_string fmt, ...);
kao_void displayBuffer(BufferReader* ptr_Buffer);
kao_long getFileSize(kao_string fname);
...
isNumber(const kao_string ns);
kao_void startReader(kao_string, kao_string, kao_char, kao_intg, kao_intg);
...
kao_intg mainReader(kao_intg argc, kao_string* argv) {

	/* Create source input buffer */
	kao_string program = argv[0];
	kao_string input = argv[2];
	kao_char mode = MODE_FIXED;
	kao_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
kao_void startReader(kao_string program, kao_string input, kao_char mode, kao_intg size, kao_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	kao_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	kao_char symbol;			/* symbol read from input file */
...
kao_void bErrorPrint(kao_string fmt, ...) {
	...
	(kao_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
kao_intg isNumber(const kao_string ns) {
	kao_char c; kao_intg i = 0;
	if (ns == NULL) return 0;
	while ((c = ns[i++]) == 0) {
