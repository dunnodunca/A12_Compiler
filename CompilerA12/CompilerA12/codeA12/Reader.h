...
typedef struct offset {
	kao_intg mark;			/* the offset to the mark position (in chars) */
	kao_intg read;			/* the offset to the get a char position (in chars) */
	kao_intg wrte;			/* the offset to the add chars (in chars) */
} Offset;

/* Buffer structure */
typedef struct bufferReader {
	kao_string	content;			/* pointer to the beginning of character array (character buffer) */
	kao_intg	size;				/* current dynamic memory size (in bytes) allocated to character buffer */
	kao_intg	increment;			/* character array increment factor */
	kao_intg	mode;				/* operational mode indicator */
	kao_byte	flags;				/* contains character array reallocation flag and end-of-buffer flag */
	Offset		offset;				/* Offset / position field */
	kao_intg	histogram[NCHAR];	/* Statistics of chars */
	kao_intg	numReaderErrors;	/* Number of errors from Reader */
} BufferReader, * ReaderPointer;

/* FUNCTIONS DECLARATION:  .................................. */
/* General Operations */
ReaderPointer	readerCreate		(kao_intg, kao_intg, kao_intg);
...
kao_boln		readerSetMark		(ReaderPointer const, kao_intg);
kao_intg		readerPrint		    (ReaderPointer const);
kao_intg		readerLoad			(ReaderPointer const, FILE* const);
...
kao_string		readerGetContent	(ReaderPointer const, kao_intg);
kao_intg		readerGetPosRead	(ReaderPointer const);
kao_intg		readerGetPosWrte	(ReaderPointer const);
kao_intg		readerGetPosMark	(ReaderPointer const);
kao_intg		readerGetSize		(ReaderPointer const);
kao_intg		readerGetInc		(ReaderPointer const);
kao_intg		readerGetMode		(ReaderPointer const);
kao_byte		readerGetFlags		(ReaderPointer const);
kao_void		readerPrintStat		(ReaderPointer const);
kao_intg		readerNumErrors		(ReaderPointer const);
