 ...
 * -------------------------------------------------------------
 */
mold_void bErrorPrint(mold_string fmt, ...);
mold_void displayBuffer(BufferReader* ptr_Buffer);
mold_long getFileSize(mold_string fname);
mold_intg isNumber(const mold_string ns);
mold_void startReader(mold_string, mold_string, mold_char, mold_intg, mold_intg);
...
mold_intg mainReader(mold_intg argc, mold_string* argv) {

	/* Create source input buffer */
	mold_string program = argv[0];
	mold_string input = argv[2];
	mold_char mode = MODE_FIXED;
	mold_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
mold_void startReader(mold_string program, mold_string input, mold_char mode, mold_intg size, mold_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	mold_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	mold_char symbol;			/* symbol read from input file */
...
mold_void bErrorPrint(mold_string fmt, ...) {
	...
	(mold_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
