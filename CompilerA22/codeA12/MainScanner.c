...
kao_int errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern kao_int line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(kao_void);
 ...
 * -------------------------------------------------------------
 */
kao_void printScannerError(kao_string fmt, ...);
kao_void displayScanner(BufferReader* ptrBuffer);
kao_long getScannerFilesize(kao_string fname);
...
kao_int mainScanner(kao_int argc, kao_string* argv) {
	...
	printf("\n----------------------------------\n");
	printf("Number of scanner error: %d", errorNumber);
	printf("\n----------------------------------\n");
