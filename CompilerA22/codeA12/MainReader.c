...
kao_void startReader(kao_string, kao_string, kao_char, kao_int, kao_int);
...
kao_int mainReader(kao_int argc, kao_string* argv) {
	...
	kao_int size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
kao_void startReader(kao_string program, kao_string input, kao_char mode, kao_int size, kao_int increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	kao_int loadSize = 0;		/* the size of the file loaded in the buffer */
...
kao_int isNumber(const kao_string ns) {
	kao_char c; kao_int i = 0;
	if (ns == NULL) return 0;
	while ((c = ns[i++]) == 0) {
